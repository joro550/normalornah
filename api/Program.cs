...
app.MapGet(
    "/questions/random",
    async (
        IQuestionCacheRetriever retriever,
    ...
    {
        var questionIds = (await retriever.GetQuestionIds()).ToList();

        // If we have seen this user before then we don' want to show them
        // questions they've all ready answered
        if (userId != null)
        {
            var answers = await client.From<Answer>().Where(x => x.UserId == userId.Value).Get();

            var answeredQuestions = answers.Models.Select(x => x.QuestionId).ToList();
